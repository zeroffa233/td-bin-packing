...
本文档基于 `算法流程.md` 绘制完整程序流程，而不是只描述核心装箱算法内部过程。
...
- `快递正常路径`
    - 这里只做预处理，不存在“跳过 `lclRules` 的纯正常装箱主线”。
...
| 输入标准化 | `quantity`, `csQty`                | 若 `csQty != None`，则 `quantity % csQty`                  | 余数才进入调度           |
...
- 该路径是否仍需遵守 `quantity / maxWeight / capacityRate`，目前在接口文档中没有完全说死，属于后续待确认项。
